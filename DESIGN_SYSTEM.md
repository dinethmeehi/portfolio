...
A procedurally generated (not stock) dark image with a soft off-white radial glow and fine film grain, generated via Python/PIL:

- Base near-black fill, 2–3 large soft radial "glow" ellipses blurred heavily (Gaussian blur ~60px), a vignette, then fine Gaussian noise added as grain.
- Rendered at 1920×1200, exported as JPEG (quality ~88) and embedded as a base64 `background-image` on a fixed, full-viewport `#bg-layer` div (`z-index:-2`), with a separate `#bg-overlay` gradient div (`z-index:-1`) on top for text contrast.
- **Interactive:** on scroll, the layer's `rotate`, `scale`, and `brightness` (CSS `filter`) all interpolate based on scroll progress (0→1 down the page). On mousemove, it also translates slightly for a parallax feel. All driven by a single `requestAnimationFrame`-throttled `updateBackground()` function, skipped entirely if `prefers-reduced-motion: reduce`.
